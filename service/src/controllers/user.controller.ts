...
export const updateUser = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const user = await userService.update(paramsResult.data.id, bodyResult.data);
  successResponse(res, user);
});
...
export const assignRole = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const paramsResult = userIdSchema.safeParse(req.params);
  if (!paramsResult.success) {
    badRequestResponse(res, 'Invalid user ID');
    return;
  ...
  for (const roleId of bodyResult.data.roleIds) {
    await userService.assignRole(paramsResult.data.id, roleId);
  }
  successResponse(res, { message: 'Roles assigned successfully' });
});

export const removeRole = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const paramsResult = userIdSchema.safeParse(req.params);
  if (!paramsResult.success) {
    badRequestResponse(res, 'Invalid user ID');
    return;
